// pages/multipoint_multipoint_transmission-multiToMulti.tsx
'use client';
...
import { multipoint_multipoint_transmissionMultiToMultiConfig } from '@/app/constellation_simulation/input_format';
import {
  useMultiToMultiData,
  useSimulation,
  useDownloadResult
} from '@/app/constellation_simulation/routing/multipoint_multipoint_transmission/history/service';
...
export default function OverViewPage() {
  ...
  const [lastMultiToMultiStatus, setLastMultiToMultiStatus] = useState(null);
  const [error, setError] = useState('');
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [duplicateWarning, setDuplicateWarning] = useState(false);
  const [duplicateMultiToMultiId, setDuplicateMultiToMultiId] = useState(null);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (duplicateExperiment) {
        ...
        setDuplicateMultiToMultiId(duplicateExperiment.multiToMulti_uid);
      ...
      const payload = {
        multiToMulti_name: multiToMultiName,
      ...
      const response = await postAPI(
        `meta_data_mgt/multiToMultiManager/create_multiToMulti`,
        payload
      );
      ...
      if (response.data.status === 'success') {
        ...
        // 立即執行模擬
        try {
          await runSimulation(response.data.data.multiToMulti_uid);
          // 重新抓取 multiToMulti 資料
          await fetchMultiToMultiData();
          setShowToast(true);
        } catch (simError) {
  ...
  const handleDownloadResult = async () => {
    ...
    // 找出 id 最大的記錄
    const latestMultiToMulti = applications.reduce((prev, current) => {
      return prev.id > current.id ? prev : current;
    });

    try {
      await downloadResult(latestMultiToMulti.multiToMulti_uid);
    } catch (error) {
      setError('下載結果失敗');
    }
  };
  ...
  const handleHistoryClick = () => {
    // 前往歷史紀錄頁面
    router.push(
      `/constellation_simulation/routing/multipoint_multipoint_transmission/history`
    );
  };

  const generateMultiToMultiName = () => {
    // 使用 Math.random() 搭配 toString(36) 產生 Base36 字串
    const randomStr = Math.random().toString(36).substring(2, 8);
    return `multiToMulti_${randomStr}`;
  };
  ...
  const canDownloadResult = () => {
    ...
    return latestMultiToMulti.multiToMulti_status === 'completed';
  };

  useEffect(() => {
    if (!applications || applications.length === 0) {
      setLastMultiToMultiStatus(null);
      return;
    }

    if (duplicateWarning) {
      setLastMultiToMultiStatus({
    ...
    const status = statusMap[latestMultiToMulti.multiToMulti_status];
        ...
        <div className="mx-auto min-h-screen bg-black px-40 pt-32">
          <div className="mx-auto max-w-4xl">
            ...
            <SimulationForm
              ...
              config={multipoint_multipoint_transmissionMultiToMultiConfig}
            />
          </div>
