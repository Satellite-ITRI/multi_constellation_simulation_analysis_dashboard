...
const ApplicationCard = ({ data, onRefresh }) => {
  ...
  const handleDeleteConfirm = async () => {
    const success = await deleteGso(data.gso_uid);
    if (success) {
      onRefresh?.(); // 調用父組件傳入的刷新函數
    }
    setIsDeleteModalOpen(false);
  };
  const getCellUtLabel = (cellUt) => {
    if (!cellUt) return '';
    // 將 "35Cell_" 字串換成 "38Cell_"
    return cellUt.replace('35Cell_', '38Cell_');
  };
  ...
  const handleDetailsClick = async () => {
    const success = await downloadResult(data.gso_uid);
    if (!success) {
      // 可以加入錯誤處理，例如顯示錯誤提示
      console.error('下載失敗');
    }
  ...
  const handleRunSimulation = async () => {
    try {
      const result = await runSimulation(data.gso_uid);
      ...
      <div className="relative flex flex-col rounded-lg bg-secondary p-6 shadow-md">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <h2 className="text-xl font-semibold">{data.gso_name}</h2>
            {getStatusBadge(data.gso_status)}
          </div>
          <div className="space-x-2">
            {/* 查看結果按鈕 */}
            {data.gso_status === 'completed' && (
              <button
            ...
            <button
              ...
              disabled={isDeleting || data.gso_status === 'processing'}
        ...
        <div className="mt-4 grid grid-cols-2 gap-4">
          <div>
            ...
            <p className="font-medium">
              {getConstellationLabel(data.gso_parameter.constellation)}
            </p>
          </div>
          <div>
            <p className="text-sm text-gray-500">換手策略</p>
            <p className="font-medium">
              {data.gso_parameter.handover_strategy}
            </p>
          </div>
          <div>
            <p className="text-sm text-gray-500">換手時機</p>
            <p className="font-medium">
              {data.gso_parameter.handover_decision}
            </p>
          </div>
          <div>
            <p className="text-sm text-gray-500">Cell/UT 配置</p>
            <p className="font-medium">
              {getCellUtLabel(data.gso_parameter.cell_ut)}
            </p>
          </div>
          <div>
            <p className="text-sm text-gray-500">波束數量</p>
            <p className="font-medium">
              {data.gso_parameter.beams_per_satellite}
            </p>
          </div>
          <div>
            <p className="text-sm text-gray-500">衛星頻率數</p>
            <p className="font-medium">
              {data.gso_parameter.frequencies_per_satellite}
            </p>
          </div>
        </div>

        <div className="mt-4 flex justify-between text-sm text-gray-500">
          <span>建立時間: {getTimeAgo(data.gso_create_time)}</span>
          <span>更新時間: {getTimeAgo(data.gso_update_time)}</span>
        </div>
      </div>
