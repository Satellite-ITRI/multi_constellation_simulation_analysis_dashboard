...
import { gso_protectionGsoConfig } from '@/app/constellation_simulation/input_format';
import {
  useGsoData,
  useSimulation,
  useDownloadResult
...
export default function OverViewPage() {
  ...
  const [lastGsoStatus, setLastGsoStatus] = useState(null);
  const [error, setError] = useState('');
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [duplicateWarning, setDuplicateWarning] = useState(false);
  const [duplicateGsoId, setDuplicateGsoId] = useState(null);
  const [statusKey, setStatusKey] = useState(0);
  const [formData, setFormData] = useState(
    gso_protectionGsoConfig.defaultValues
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const keysToCheck = Object.keys(gso_protectionGsoConfig.defaultValues);

      // 檢查重複實驗
      const duplicateExperiment = applications?.find((app) => {
        const params = app.gso_parameter || {};
      ...
      if (duplicateExperiment) {
        ...
        setDuplicateGsoId(duplicateExperiment.gso_uid);
      ...
      // 產生 gso_xxx 的隨機名稱
      const gsoName = generateGsoName();

      // 將 formData 的欄位動態組成 gso_parameter
      const gso_parameter = Object.entries(formData).reduce(
        (acc, [key, value]) => {
      ...
      const response = await postAPI(
        `meta_data_mgt/gsoManager/create_gso`,
        payload
      );
      ...
      if (response.data.status === 'success') {
        ...
        // 立即執行模擬
        try {
          await runSimulation(response.data.data.gso_uid);
          // 重新抓取 gso 資料
          await fetchGsoData();
          setShowToast(true);
        } catch (simError) {
  ...
  const handleDownloadResult = async () => {
    ...
    // 找出 id 最大的記錄
    const latestGso = applications.reduce((prev, current) => {
      return prev.id > current.id ? prev : current;
    });

    try {
      await downloadResult(latestGso.gso_uid);
    } catch (error) {
      setError('下載結果失敗');
    }
  };
  ...
  const handleHistoryClick = () => {
    // 前往歷史紀錄頁面
    router.push(`/constellation_simulation/handover/gso_protection/history`);
  };

  const generateGsoName = () => {
    // 使用 Math.random() 搭配 toString(36) 產生 Base36 字串
    const randomStr = Math.random().toString(36).substring(2, 8);
    return `gso_${randomStr}`;
  };
  ...
  const canDownloadResult = () => {
    ...
    return latestGso.gso_status === 'completed';
  };

  useEffect(() => {
    if (!applications || applications.length === 0) {
      setLastGsoStatus(null);
      return;
    }
    ...
    const status = statusMap[latestGso.gso_status];
          ...
          <div className="mx-auto max-w-4xl">
            <div className="mb-6 flex items-center justify-between">
              <h1 className="flex items-center text-2xl font-bold">
                ...
                <div key={statusKey}>
                  {lastGsoStatus && (
                    <span className={lastGsoStatus.style}>
                      {lastGsoStatus.status}
                    </span>
                  )}
                </div>
              </h1>
            ...
            <SimulationForm
              ...
              config={gso_protectionGsoConfig}
            />
          </div>
