      ...
      <div className="relative flex flex-col rounded-lg bg-white p-6 shadow-md">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <h2 className="text-xl font-semibold">{data.isl_name}</h2>
            {getStatusBadge(data.isl_status)}
          </div>
          <div className="space-x-2">
            {/* 查看結果按鈕 */}
            {data.isl_status === 'completed' && (
              <button
            ...
            <button
              ...
              disabled={isDeleting || data.isl_status === 'processing'}
        ...
        <div className="mt-4 grid grid-cols-2 gap-4">
          <div>
            ...
            <p className="font-medium">{data.isl_parameter.constellation}</p>
          </div>
          <div>
            <p className="text-sm text-gray-500">連結方法</p>
            <p className="font-medium">{data.isl_parameter.isl_link_method}</p>
          </div>
          <div>
            <p className="text-sm text-gray-500">執行函數</p>
            <p className="font-medium">{data.isl_parameter.execute_function}</p>
          </div>
          <div>
            <p className="text-sm text-gray-500">地面站位置</p>
            <p className="font-medium">
              {`(${data.isl_parameter.station_location_latitude.toFixed(6)}, 
              ${data.isl_parameter.station_location_longitude.toFixed(6)}, 
              ${data.isl_parameter.station_location_altitude.toFixed(6)})`}
            </p>
          </div>
        </div>

        <div className="mt-4 flex justify-between text-sm text-gray-500">
          <span>建立時間: {getTimeAgo(data.isl_create_time)}</span>
          <span>更新時間: {getTimeAgo(data.isl_update_time)}</span>
        </div>
      </div>
