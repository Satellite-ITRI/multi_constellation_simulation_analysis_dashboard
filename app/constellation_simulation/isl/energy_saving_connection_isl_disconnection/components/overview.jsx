// pages/energy_saving_connection_isl_disconnection-islHopping.tsx
'use client';
...
import { energy_saving_connection_isl_disconnectionIslHoppingConfig } from '@/app/constellation_simulation/input_format';
import {
  useIslHoppingData,
  useSimulation,
  useDownloadResult
} from '@/app/constellation_simulation/isl/energy_saving_connection_isl_disconnection/history/service';
...
export default function OverViewPage() {
  ...
  const [lastIslHoppingStatus, setLastIslHoppingStatus] = useState(null);
  const [error, setError] = useState('');
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [duplicateWarning, setDuplicateWarning] = useState(false);
  const [duplicateIslHoppingId, setDuplicateIslHoppingId] = useState(null);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (duplicateExperiment) {
        ...
        setDuplicateIslHoppingId(duplicateExperiment.islHopping_uid);
      ...
      const payload = {
        islHopping_name: islHoppingName,
      ...
      const response = await postAPI(
        `meta_data_mgt/islHoppingManager/create_islHopping`,
        payload
      );
      ...
      if (response.data.status === 'success') {
        ...
        // 立即執行模擬
        try {
          await runSimulation(response.data.data.islHopping_uid);
          // 重新抓取 islHopping 資料
          await fetchIslHoppingData();
          setShowToast(true);
        } catch (simError) {
  ...
  const handleDownloadResult = async () => {
    ...
    // 找出 id 最大的記錄
    const latestIslHopping = applications.reduce((prev, current) => {
      return prev.id > current.id ? prev : current;
    });

    try {
      await downloadResult(latestIslHopping.islHopping_uid);
    } catch (error) {
      setError('下載結果失敗');
    }
  };
  ...
  const handleHistoryClick = () => {
    // 前往歷史紀錄頁面
    router.push(
      `/constellation_simulation/isl/energy_saving_connection_isl_disconnection/history`
    );
  };

  const generateIslHoppingName = () => {
    // 使用 Math.random() 搭配 toString(36) 產生 Base36 字串
    const randomStr = Math.random().toString(36).substring(2, 8);
    return `islHopping_${randomStr}`;
  };
  ...
  const canDownloadResult = () => {
    ...
    return latestIslHopping.islHopping_status === 'completed';
  };

  useEffect(() => {
    if (!applications || applications.length === 0) {
      setLastIslHoppingStatus(null);
      return;
    }

    if (duplicateWarning) {
      setLastIslHoppingStatus({
    ...
    const status = statusMap[latestIslHopping.islHopping_status];
          ...
          <div className="mx-auto max-w-4xl">
            <div className="mb-6 flex items-center justify-between">
              <h1 className="flex items-center text-2xl font-bold">
                節能連線ISL斷線
                <div key={statusKey}>
                  {lastIslHoppingStatus && (
                    <span className={lastIslHoppingStatus.style}>
                      {lastIslHoppingStatus.status}
                    </span>
                  )}
                </div>
              </h1>
            ...
            <SimulationForm
              ...
              config={
                energy_saving_connection_isl_disconnectionIslHoppingConfig
              }
            />
          </div>
