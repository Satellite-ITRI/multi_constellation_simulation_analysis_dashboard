...
const ApplicationCard = ({ data, onRefresh }) => {
  ...
  const {
    ...
    toastType,
    toastMessage
  } = useDeleteConstellationStrategy();

  const handleDeleteConfirm = async () => {
    const success = await deleteConstellationStrategy(
      data.constellationStrategy_uid
    );
    if (success) {
  ...
  const handleDetailsClick = async () => {
    const success = await downloadResult(data.constellationStrategy_uid);
    if (!success) {
      // 可以加入錯誤處理，例如顯示錯誤提示
      console.error('下載失敗');
    }
  ...
  const handleRunSimulation = async () => {
    try {
      const result = await runSimulation(data.constellationStrategy_uid);
    ...
    <>
      <div className="relative flex flex-col rounded-lg bg-secondary p-6 shadow-md">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <h2 className="text-xl font-semibold">
              {data.constellationStrategy_name}
            </h2>
            {getStatusBadge(data.constellationStrategy_status)}
          </div>
          <div className="space-x-2">
            {/* 查看結果按鈕 */}
            {data.constellationStrategy_status === 'completed' && (
              <button
            ...
            <button
              ...
              disabled={
                isDeleting || data.constellationStrategy_status === 'processing'
              }
          ...
          <div>
            ...
            <p className="font-medium">
              {getConstellationLabel(
                data.constellationStrategy_parameter.TLE_inputFileName
              )}
            </p>
          </div>
          <div>
        ...
        <div className="mt-4 flex justify-between text-sm text-gray-500">
          <span>
            建立時間: {getTimeAgo(data.constellationStrategy_create_time)}
          </span>
          <span>
            更新時間: {getTimeAgo(data.constellationStrategy_update_time)}
          </span>
        </div>
      </div>
