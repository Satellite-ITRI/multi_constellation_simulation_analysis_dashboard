// pages/phase_parameter_selection-phase.tsx
'use client';
...
import { phase_parameter_selectionPhaseConfig } from '@/app/constellation_simulation/input_format';
import {
  usePhaseData,
  useSimulation,
  useDownloadResult
} from '@/app/constellation_simulation/constellation/phase_parameter_selection/history/service';
...
export default function OverViewPage() {
  ...
  const [lastPhaseStatus, setLastPhaseStatus] = useState(null);
  const [error, setError] = useState('');
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [duplicateWarning, setDuplicateWarning] = useState(false);
  const [duplicatePhaseId, setDuplicatePhaseId] = useState(null);
  const [statusKey, setStatusKey] = useState(0);
  const [formData, setFormData] = useState(
    phase_parameter_selectionPhaseConfig.defaultValues
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const duplicateExperiment = applications?.find((app) => {
        const params = app.phase_parameter || {};
      ...
      if (duplicateExperiment) {
        ...
        setDuplicatePhaseId(duplicateExperiment.phase_uid);
      ...
      // 產生 phase_xxx 的隨機名稱
      const phaseName = generatePhaseName();

      // 將 formData 的欄位動態組成 phase_parameter
      const phase_parameter = Object.entries(formData).reduce(
        (acc, [key, value]) => {
      ...
      const response = await postAPI(
        `meta_data_mgt/phaseManager/create_phase`,
        payload
      );
      ...
      if (response.data.status === 'success') {
        ...
        // 立即執行模擬
        try {
          await runSimulation(response.data.data.phase_uid);
          // 重新抓取 phase 資料
  ...
  const handleDownloadResult = async () => {
    ...
    // 找出 id 最大的記錄
    const latestPhase = applications.reduce((prev, current) => {
      return prev.id > current.id ? prev : current;
    });

    try {
      await downloadResult(latestPhase.phase_uid);
    } catch (error) {
      setError('下載結果失敗');
    }
  };
  ...
  const handleHistoryClick = () => {
    // 前往歷史紀錄頁面
    router.push(
      `/constellation_simulation/constellation/phase_parameter_selection/history`
    );
  };
  ...
  const canDownloadResult = () => {
    ...
    return latestPhase.phase_status === 'completed';
  };

  useEffect(() => {
    if (!applications || applications.length === 0) {
      setLastPhaseStatus(null);
      return;
    }
    ...
    const status = statusMap[latestPhase.phase_status];
          ...
          <div className="mx-auto max-w-4xl">
            <div className="mb-6 flex items-center justify-between">
              <h1 className="flex items-center text-2xl font-bold">
                ...
                <div key={statusKey}>
                  {lastPhaseStatus && (
                    <span className={lastPhaseStatus.style}>
            ...
            <SimulationForm
              ...
              config={phase_parameter_selectionPhaseConfig}
            />
          </div>
